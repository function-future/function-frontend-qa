@Core @ActivityBlogs @Mentor @MentorActivityBlogs @Regression
Feature: Activity Blogs Page with role as Mentor
  ...
  @Positive @MentorVisitActivityBlogsPage
  Scenario: mentor visit activity blogs page
  ...
  @Positive @MentorSuccessCreateActivityBlog
  Scenario: mentor create activity blog
    Given user should be logged in
    And user should see menu bar
    And user should see activity blogs menu on row 3
    When user click activity blogs menu on row 3
  ...
  @Positive @MentorClickActivityBlogOnActivityBlogs
  Scenario: mentor click on an activity blog on activity blogs page and redirect to activity blog detail page
    Given user should be logged in
    And user should see menu bar
    And user should see activity blogs menu on row 3
    When user click activity blogs menu on row 3
